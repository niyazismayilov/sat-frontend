...
import { Enum_Course_Durationtype, useCourseRegistersQuery } from 'graphql/generated';
...
export const AllCourses: React.FC = () => {
    ...
    const { data, loading } = useCourseRegistersQuery({
        variables: {
            filters: {
                user: {
                    id: {
        ...
        <Root>
            <Grid container spacing={2.5}>
                {courseRegisters?.map((courseRegister) => {
                    const course = courseRegister.attributes?.group?.data?.attributes?.course?.data?.attributes;
                    const trainers =
                        courseRegister.attributes?.group?.data?.attributes?.course?.data?.attributes?.trainers?.data;
                    return (
                        <Grid item xs={12} sm={10} md={10} lg={6} key={courseRegister.id}>

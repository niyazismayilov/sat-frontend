...
import { useVideoFilterDispatch } from 'context/useful/video/store';
...
export const VideosActions: React.FC<CustomersActionsProps> = ({ openFilterDialog }) => {
    const dispatch = useVideoFilterDispatch();
    ...
    return (
        <Root>
            <ManagerPageHeader title="Video">
                <Box>
                    <Divider />
                </Box>
                {isMobile ? (
                    <>
                        <Tooltip title="Yeni video" arrow>
                            <IconButton sx={{ ml: 3 }} onClick={() => browserHistory.push('/d/helpful/video/create')}>
                                <AddIcon />
                            </IconButton>
                        </Tooltip>
                    ...
                    <>
                        <Link to="/d/helpful/video/create">
                            <Button
                                startIcon={<PlusIcon />}
                                variant="contained"
                                sx={{ mr: 12 }}
                        ...
                        <Formik
                            initialValues={{ search: filter?.title || '' }}
                            innerRef={formikRef}
                            onSubmit={() => {
                                dispatch({
                                    type: 'SET_FILTER',
                            ...
                            {({ handleSubmit }) => (
                                <Form onSubmit={handleSubmit}>
                                    <Field
                                        name="search"
                                        component={TextField}
                                        placeholder="Video adına görə axtar"
                                        variant="outlined"
                                        InputProps={{
                                            startAdornment: (
                                                <InputAdornment

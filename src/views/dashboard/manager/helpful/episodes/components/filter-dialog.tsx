...
import { INITIAL_BROADCASTS_FILTER_STATE } from 'context/useful/broadcast/reducer';
import { useBroadcastsFilter } from 'context/useful/broadcast/store';
import { Field, Form, Formik } from 'formik';
import { TextField } from 'formik-mui';
import { Enum_Broadcast_Status } from 'graphql/generated';
...
export const FilterDialog: React.FC<FilterDialogProps> = ({ filterDialogOpen, onClose }) => {
    ...
    const [{ filter }, dispatch] = useBroadcastsFilter();
    const isMobile = useMediaQuery(theme.breakpoints.down('xs'));
    const paginationDispatch = usePaginationDispatch();
    const initialValues: any = {
        ...INITIAL_BROADCASTS_FILTER_STATE.filter,
        ...filter,
    };
                        ...
                        <Form onSubmit={handleSubmit}>
                            <DialogContent className="dialog-content">
                                <Grid container spacing={3}>
                                    <Grid item xs={12}>
                                        <Field name="title" component={TextField} label="Epizodun adı" fullWidth />
                                    </Grid>
                                ...
                                <Grid item xs={12}>
                                    <Box className="actions">
                                        <Button
                                            ...
                                            onClick={() =>
                                                resetForm({
                                                    values: INITIAL_BROADCASTS_FILTER_STATE.filter as any,
                                                })
                                            }
                                        >
                                            Sıfırla

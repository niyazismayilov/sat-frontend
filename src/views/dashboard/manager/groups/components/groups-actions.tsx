...
import { useGroupFilterDispatch } from 'context/group-filter/store';
...
export const GroupActions: React.FC<GroupsActionsProps> = ({ openFilterDialog }) => {
    const dispatch = useGroupFilterDispatch();
    ...
    return (
        <Root>
            <ManagerPageHeader title="Qrup">
                <Box>
                    <Divider />
                </Box>
                {isMobile ? (
                    <>
                        <Tooltip title="Yeni qrup" arrow>
                            <IconButton sx={{ ml: 3 }} onClick={() => browserHistory.push('/d/training/groups/create')}>
                                <AddIcon />
                            </IconButton>
                        </Tooltip>
                    ...
                    <>
                        <Link to="/d/training/groups/create">
                            <Button
                                startIcon={<PlusIcon />}
                                variant="contained"
                                sx={{ mr: 12 }}
                            ...
                            {({ handleSubmit }) => (
                                <Form onSubmit={handleSubmit}>
                                    <Field
                                        name="search"
                                        component={TextField}
                                        placeholder="Qrupun adına görə axtar"
                                        variant="outlined"
                                        InputProps={{
                                            startAdornment: (
                                                <InputAdornment

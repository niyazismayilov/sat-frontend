...
import { INITIAL_TRAINER_FILTER_STATE } from 'context/trainer-filter/reducer';
import { useTrainerFilter } from 'context/trainer-filter/store';
import { Field, Form, Formik } from 'formik';
import { TextField } from 'formik-mui';
import { Enum_Trainer_Status, useCoursesQuery } from 'graphql/generated';
...
export const FilterDialog: React.FC<FilterDialogProps> = ({ filterDialogOpen, onClose }) => {
    ...
    const [{ filter }, dispatch] = useTrainerFilter();
    const isMobile = useMediaQuery(theme.breakpoints.down('xs'));
    const paginationDispatch = usePaginationDispatch();
    const initialValues: any = {
        ...INITIAL_TRAINER_FILTER_STATE.filter,
        ...filter,
    };
    ...
    return (
        <Dialog open={filterDialogOpen} onClose={onClose} fullScreen={isMobile} maxWidth="xs">
            ...
            >
                {({ handleSubmit, resetForm, setFieldValue, values }): React.ReactNode => {
                                ...
                                <Grid container spacing={3}>
                                    <Grid item xs={12}>
                                        <Field
                                            name="fullName"
                                            component={TextField}
                                            label="Təlimçi adı və ya soyadına görə axtar"
                                            fullWidth
                                        />
                                    </Grid>
                                    <Grid item xs={12}>
                                ...
                                <Grid item xs={12}>
                                    <Box className="actions">
                                        <Button
                                            ...
                                            onClick={() =>
                                                resetForm({
                                                    values: INITIAL_TRAINER_FILTER_STATE.filter as any,
                                                })
                                            }
                                        >
                                            Sıfırla

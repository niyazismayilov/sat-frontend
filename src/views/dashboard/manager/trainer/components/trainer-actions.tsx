...
import { useTrainerFilterDispatch } from 'context/trainer-filter/store';
...
export const TrainersActions: React.FC<CustomersActionsProps> = ({ openFilterDialog }) => {
    const dispatch = useTrainerFilterDispatch();
    ...
    return (
        <Root>
            <ManagerPageHeader title="Təlimçi">
                <Box>
                    <Divider />
                </Box>
                {isMobile ? (
                    <>
                        <Tooltip title="Yeni təlimçi" arrow>
                            <IconButton sx={{ ml: 3 }} onClick={() => browserHistory.push('/d/trainers/create')}>
                                <AddIcon />
                            </IconButton>
                        </Tooltip>
                            ...
                            {({ handleSubmit }) => (
                                <Form onSubmit={handleSubmit}>
                                    <Field
                                        name="search"
                                        component={TextField}
                                        placeholder="Təlimçi ad soyadına görə axtar"
                                        variant="outlined"
                                        InputProps={{
                                            startAdornment: (
                                                <InputAdornment

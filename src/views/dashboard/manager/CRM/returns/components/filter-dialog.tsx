...
import { INITIAL_CALLBACK_FILTER_STATE } from 'context/callbacks-filter/reducer';
import { useCallbacksFilter } from 'context/callbacks-filter/store';
...
export const FilterDialog: React.FC<FilterDialogProps> = ({ filterDialogOpen, onClose }) => {
    ...
    const [{ filter }, dispatch] = useCallbacksFilter();
    const isMobile = useMediaQuery(theme.breakpoints.down('xs'));
    const paginationDispatch = usePaginationDispatch();
    const initialValues: any = {
        ...INITIAL_CALLBACK_FILTER_STATE.filter,
        ...filter,
    };
                                    ...
                                    <Grid container spacing={3}>
                                        ...
                                        <Grid item xs={12} sm={6}>
                                            <Field
                                                ...
                                                fullWidth
                                            >
                                                <MenuItem value={isCalledEnum.is_Called}>Zəng edildi</MenuItem>
                                                <MenuItem value={isCalledEnum.is_Not_Called}>Zəng edilmədi</MenuItem>
                                            </Field>
                                        </Grid>
                                        <Grid item xs={12} sm={6}>
                                            <Field
                                    ...
                                    <Grid item xs={12}>
                                        <Box className="actions">
                                            <Button
                                                ...
                                                onClick={() =>
                                                    resetForm({
                                                        values: INITIAL_CALLBACK_FILTER_STATE.filter as any,
                                                    })
                                                }
                                            >
                                                Sıfırla

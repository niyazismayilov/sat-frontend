...
import { useSubscribersFilterDispatch } from 'context/subscribers-filter/store';
...
export const SubscribersActions: React.FC<SubscribersActionsProps> = ({ openFilterDialog }) => {
    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down('lg'));
    const formikRef = useRef<FormikProps<any>>(null);
    const paginationDispatch = usePaginationDispatch();
    const dispatch = useSubscribersFilterDispatch();
    const filter = extractFilterFromQS();
    return (
        <Root>
            <ManagerPageHeader title="Abunəçilər">
                <Box>
                    <Divider />
                </Box>
                {isMobile ? (
                    <>
                        <Tooltip title="Yeni təlimçi" arrow>
                            <IconButton sx={{ ml: 3 }} onClick={() => console.log('/')}>
                                <AddIcon />
                            </IconButton>
                        </Tooltip>
                    ...
                    <>
                        <Button variant="contained" sx={{ mr: 12, p: 0 }} className="export-button">
                            <ExcelExport tableId="subscribersTable" fileName="subscribers" />
                        </Button>
                        <Formik
                            initialValues={{ search: filter?.email || '' }}
                            innerRef={formikRef}
                            onSubmit={() => {
                                dispatch({
                                    type: 'SET_FILTER',

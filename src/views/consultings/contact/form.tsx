...
import { Enum_Message_Type, useCreateMessageMutation } from 'graphql/generated';
...
export const Form: React.FC<BoxProps & { courseId?: string; closeDialog?: () => void }> = ({
    ...
    const contents = t('consulting:content');
    const newContent = contents.split('&nbsp;');
    ...
    const onSubmit = (formData, { setSubmitting }) => {
        setSubmitting(false);
        const { fullName, email, phoneNumber, consultings, type, message } = formData;

        const formInfo = {
            fullName,
            interestedConsultings: consultings,
            type,
            message,
        };
    ...
    const initialValues = {
        ...
        email: user?.email || '',
        type: Enum_Message_Type.Consulting,
        message: '',
    };
                    ...
                    return (
                        <form onSubmit={handleSubmit}>
                            <Box className="form-wrapper">
                                ...
                                <Typography sx={{ width: '100%', fontWeight: 500, marginBottom: 1 }} className="text">
                                    Salam!
                                </Typography>
                                ...
                                <MuiTextField
                                    ...
                                    SelectProps={{
                                        multiple: true,
                                        value: values.consultings,
                                        onChange: (event) => {
                                            setFieldValue('consultings', event.target.value);
                                        },
                                    }}
                                    className={errors.consultings && 'courses-input'}
                                >
